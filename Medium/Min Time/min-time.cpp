...
class Solution
{
	...
	{
		if (ind == arr.size()) return (long long) abs(curr);
		...
		if (curr > mp[arr[ind]].first && curr < mp[arr[ind]].second)
			temp = min(2 *(long long) mp[arr[ind]].second - (long long) curr - (long long) mp[arr[ind]].first + mine(ind + 1,
					mp[arr[ind]].first, dp),
				(long long) curr - 2 *(long long) mp[arr[ind]].first + (long long) mp[arr[ind]].second + mine(ind + 1,
					mp[arr[ind]].second, dp));
